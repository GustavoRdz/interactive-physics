    ...
    div(style="display: flex; flex-direction: row; justify-content: space-around;")
      div(style="display: flex; flex-direction: row; justify-content: space-around;")
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 300" height="300" width="600" >
          ...
          <rect x="0" y="50" width="600" height="200" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>
          <rect x="0" y="100" width="600" height="100" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>
          <rect x="0" y="150" width="600" height="1" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>
          <rect x="50" y="0" width="500" height="300" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>
          <rect x="100" y="0" width="400" height="300" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>
          <rect x="150" y="0" width="300" height="300" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>
          <rect x="200" y="0" width="200" height="300" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>
          <rect x="250" y="0" width="100" height="300" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>
          <rect x="300" y="0" width="1" height="300" fill="blue" fill-opacity="0.2" stroke="#000000"  stroke-width="0.1"/>

          <g> //- axes
            <path d="M100,200 L100,0 95,15 100,0 105,15 M190,60 L10,140 20,130 10,140 25,138 M0,78 L550,200 535,200 550,200 537,193" stroke="black" fill="none"/>
          </g>
          <defs>
            <path id="wavePath" fill="none" stroke="red">
              <animate id="waveId" attributeName="d" :values="wave" :dur="period" repeatCount="indefinite"/>
          ...
          <defs>
            <path id="waveHPath" fill="none" stroke="orange">
              <animate id="waveHId" attributeName="d" :values="waveH" :dur="period" repeatCount="indefinite"/>
              //- <animate attributeName="stroke" values="red;red;black;red;red;black;red;red;" :dur="period" repeatCount="indefinite" begin="p1.begin"/>
            </path>
          </defs>
          <use xlink:href="#waveHPath"/>
...
export default {
  ...
  computed: {
    ...
    wave () {
      ...
      // let ranges = [350, 342, 318, 282, 237, 188, 140, 98, 68, 52, 52, 68, 98, 140, 188, 237, 282, 318, 342, 350]
      let ranges = [550, 550, 550, 550, 550, 106, 65, 50, 65, 106, 167, 233, 294, 335, 350]
      let points = 200
      for (var i = 0; i < ranges.length; i++) {
        let d1 = 'M'
        // let delta = ranges[i] / (points + 0)
        let delta = 600 / (points + 0)
        // let period = ranges[i] / 7
        let period = 600 / 7
        for (var j = 0; j < points - 1; j++) {
          let x = delta * j
          let y = (40 * Math.sin(2 * Math.PI * delta * j / period - i / 2))
          let angle = 12 * Math.PI / 180
          // let alpha = 12 * Math.PI / 180
          // let beta = 12 * Math.PI / 180
          // let gamma = 12 * Math.PI / 180
          // let A = [[1, 0, 0], [0, Math.cos(alpha), Math.sin(alpha)], [0, -Math.sin(alpha), Math.cos(alpha)]]
          // let B = [[Math.cos(beta), 0, Math.sin(beta)], [0, 1, 0], [-Math.sin(beta), 0, Math.cos(beta)]]
          // let C = [[Math.cos(gamma), -Math.sin(gamma), 0], [Math.sin(gamma), Math.cos(gamma), 0], [0, 0, 1]]
          // let AB

          d1 = d1 + `${x * Math.cos(angle) - y * Math.sin(angle)},${80 + x * Math.sin(angle) + y * Math.cos(angle)} `
          // d1 = d1 + `${delta * j + 0},${Math.tan(0) * delta * j + 80 + 40 * Math.sin(2 * Math.PI * delta * j / period - i * 6)} `
        }
        d1 = d1 + ` ${600},200 ${600},100;`
        d = d + d1
      }
      return d
    },
    waveH () {
      ...
      for (var i = 0; i < ranges.length; i++) {
        ...
        for (var j = 0; j < points - 1; j++) {
          let x = delta * j
          let y = (20 * Math.sin(2 * Math.PI * delta * j / period - i / 2))
          let angle = 12 * Math.PI / 180
          d1 = d1 + `${x * Math.cos(angle) - y * Math.sin(angle)},${80 + x * Math.sin(angle) + y * Math.cos(angle)} `
        }
        d1 = d1 + ` ${600},200 ${600},100;`
