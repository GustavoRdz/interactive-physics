...
eg-transition(:enter='enter', :leave='leave')
  .eg-slide-content
    p.problem Hallar el valor de K para la recta Kx + {{ B }}y + {{ C }} = 0 que forma con los ejes coordenados un triángulo rectángulo de área igual a {{ area }} unidades de superficie
    .center
...
export default {
  ...
  computed: {
    ...
    trianguloA: function () {
      return 'M354,238 l' + `${38 * this.enterB}` + ',0 ' + `${-38 * this.enterB}` + ',' + `${-38 * this.enterA}` + 'Z'
    },
    trianguloB: function () {
      return 'M354,238 l' + `${-38 * this.enterBp}` + ',0 ' + `${+38 * this.enterBp}` + ',' + `${38 * this.enterAp}` + 'Z'
    }
  },
  methods: {
