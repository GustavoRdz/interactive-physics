  ...
  .eg-slide-content
    ...
    .center
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 300" height="300px" width="600px" >
        ...
        <!-- Parabola -->
        <path :d="hiperbola" stroke="#000" fill="none" stroke-opacity="1" stroke-width="3" ></path>
        ...
        //-   <animate attributeName="x" :values="hiperbolaX" dur="5" repeatCount="indefinite"/>
        //-   <animate attributeName="y" :values="hiperbolaY" dur="5" repeatCount="indefinite"/>
        //- </text>   
        <!-- texto L(-p,y) -->
    ...
    .center
      ...
      p.inline.data <span style="font-family: Times;">¿Potencia de x?<br>(1 o 2)</span>
        input.center.data(:class="checkedPotencia" v-model.number='enterA')
        //- <span class="error" v-if="errorX1">[e: {{ errorX1.toPrecision(3) }}%]</span>
      p.inline.data <span style="font-family: Times;">¿p?</span>
        input.center.data(:class="checkedP" v-model.number='enterB')
...
export default {
  ...
  computed: {
    ...
    hiperbola () {
      ...
      // let potencia = this.enterA === 1 ? 0.5 : 2
      // let coef = this.enterPotencia === 1 ? Math.sqrt(4 * Math.abs(this.enterP)) : 1 / (4 * Math.abs(this.enterP))
      let coef = this.enterA === 1 ? this.enterA / this.enterB : this.enterA / this.enterB
      console.log(coef)
      // coef = this.enterP < 0 ? -coef : coef
      // let coefY = this.enterPotencia === 1 ? Math.sqrt(4 * this.enterP) : 1
      for (var j = 0; j < points - 1; j++) {
        let xR = 150 + this.deltaX * (j * delta + this.enterA)
        let xL = 150 - this.deltaX * (j * delta + this.enterA)
        // let xL = 150 - this.deltaX * Math.sqrt(Math.pow(j * delta, 2) - Math.pow(this.enterA, 2)) * coef
        let yUp = 150 - this.deltaX * Math.sqrt(Math.pow(j * delta + this.enterA, 2) - Math.pow(this.enterA, 2)) * coef
        let yDn = 150 + this.deltaX * Math.sqrt(Math.pow(j * delta + this.enterA, 2) - Math.pow(this.enterA, 2)) * coef
        // let yDn = 150 + this.deltaX * Math.sqrt(Math.pow(j * delta, 2) - Math.pow(this.enterA, 2)) * coef
        this.hiperbiolaX += `${xR};`
        this.hiperbolaY += `${yUp};`
        d1 = this.enterA === 1 ? d1 + `${xR},${yUp} ` : d1 + `${xR},${yUp} `
        d2 = this.enterA === 1 ? d2 + `${xR},${yDn} ` : d2 + `${xR},${yDn} `
        d3 = this.enterA === 1 ? d3 + `${xL},${yUp} ` : d3 + `${xL},${yUp} `
        d4 = this.enterA === 1 ? d4 + `${xL},${yDn} ` : d4 + `${xL},${yDn} `
      }
      // d1 = d1 + ` ${ranges[i] + 30},90 ${ranges[i] + 50},90;`
      d = 'M' + d1 + ' M' + d2 + ' M' + d3 + ' M' + d4
      // }
      console.log(d)
      return d
    },
