    ...
    div(style="display: flex; flex-direction: row; justify-content: space-around;")
      ...
      div(style="display: flex; flex-direction: column; justify-content: flex-start;")
        div(style="border: solid 1px; border-radius: 5%; height: 97px;")
          p(style="font-family: Times; font-style: italic; font-size:20px; font-weight: bold;color: black; margin: 3px 0 10px 0;").center A = <span style="font-style: normal;">{{  parseFloat(amplitude).toPrecision(3)}}</span> m<br>
            button(@click="increaseA(0.1, 0)") +0.1
            button(@click="increaseA(-0.1, 0)") -0.1
            <br>
            button(@click="increaseA(1, 0)") +1
            button(@click="increaseA(-1, 0)") -1
        div(style="border: solid 1px; border-radius: 5%; height: 97px;")
          p(style="font-family: Times; font-style: italic; font-size:20px; font-weight: bold;color: black; margin: 3px 0 10px 0;").center &omega; = <span style="font-style: normal; font-size:1em;">{{ parseFloat(omega).toPrecision(3) }}</span> rad/s<br>
            button(@click="increaseOmega(0.1, 0)") +0.1
            button(@click="increaseOmega(-0.1, 0)") -0.1
            <br>
            button(@click="increaseOmega(1, 0)") +1.0
            button(@click="increaseOmega(-1, 0)") -1.0
        div(style="border: solid 1px; border-radius: 5%; height: 97px;")
          p(style="font-family: Times; font-size:20px; font-weight: bold;color: black; margin: 10px 0 10px 0;").center &phi; = {{ parseInt((phi * 180 * 100 / Math.PI))/100 }}º <br>
            button(@click="increasePhi(1, 0)") +1
            button(@click="increasePhi(-1, 0)") -1
            <br>
            button(@click="increasePhi(5, 0)") +5
            button(@click="increasePhi(-5, 0)") -5
...
export default {
  data: function () {
    return {
      ...
      displacementsY: '',
      dispY: 1,
      gridX: this.range(0, 600, 50),
      gridY: this.range(0, 300, 50)
    }
  },
  computed: {
    displacement () {
      let y0 = 150 - 25 * this.amplitude * Math.cos(this.phi)
      console.log(y0)
      let numPoints = 100
      let delta = 450 / numPoints

      let d = `M100,${y0} `
      this.displacementsX = ''
      this.displacementsX += '95;'
      this.displacementsY = ''
      this.displacementsY += `${y0};`
      this.dispY = [y0]
      for (var i = 1; i < numPoints; i++) {
        let x = i * delta
        this.dispY = this.amplitude * Math.cos(this.omega * x / 50 + this.phi)
        d = d + `${100 + x},${150 - 25 * this.amplitude * Math.cos(this.omega * x / 50 + this.phi)} `
        this.displacementsX += `${90 + x};`
        this.displacementsY += `${140 - 25 * this.amplitude * Math.cos(this.omega * x / 50 + this.phi)};`
      }
      console.log(d)
      return d
    },
    ...
    //     console.log('posi: ' + Math.round(250 + 150 * Math.cos(angle)))
    //     this.dispY.push(Math.round(250 + 150 * Math.cos(angle)))
    //     console.log('dispY: ' + this.dispY)
    ...
    resorteA () {
      ...
      // let ranges = [350, 342, 318, 282, 237, 188, 140, 98, 68, 52, 52, 68, 98, 140, 188, 237, 282, 318, 342, 350]
      let ranges = this.dispY // [350, 335, 294, 233, 167, 106, 65, 50, 65, 106, 167, 233, 294, 335, 350]
      console.log('Ranges')
      console.log(ranges)
      console.log(ranges.length)
      let points = ranges.length
      console.log(9 / this.period)
      console.log(Math.ceil(9 / this.period))
      // let periods = (9 / this.period)
      let d = ''
      for (var i = 0; i < ranges.length; i++) {
        ...
        for (var j = 0; j < points; j++) {
          d1 = d1 + `${100 + 20 * Math.sin(2 * Math.PI * delta * j / period)},${280 - ranges[i] - j * delta} `
          // d1 = d1 + `${100 + 20 * Math.sin(2 * Math.PI * delta * j / period)},${280 - ranges[i] * Math.cos(delta * j / 100)} `
          // d1 = d1 + `${100 + 20 * Math.sin(2 * Math.PI * delta * j / period)},${280 - 25 * this.amplitude * delta * j / 50} `
          // d1 = d1 + `${145 - 25 * this.amplitude * Math.cos(this.omega * j * delta / 50 + this.phi)},${250 - this.amplitude * delta * j / 5} `
        }
        // console.log('ranges[i]: ' + ranges[i])
        d1 = d1 + `100,${ranges[i] + 10} 100,${ranges[i]};`
        d = d + d1
      }
      console.log(d)
      return d
    ...
    pathRecorrido () {
      let d = 'M0,0 450,0'
      // let pers = Math.floor(9 / this.period)
      // d += `${pers * 50 * this.period},0;`
      // console.log(d)
      return d
    },
    // displacement () {
  ...
  methods: {
    ...
    increaseOmega (l, timeScale) {
      ...
      if (this.omega + l < 0.1) {
        ...
        this.omega = this.omega + l
      }
      // this.omega = this.prod * Math.sqrt(this.elasticK / this.mass)
      this.period = 1 * 2 * Math.PI / this.omega
      console.log(this.period)
    },
    increasePhi (l, timeScale) {
      if (this.prod + timeScale < 0.1) {
        this.prod = 0.1
      } else {
        this.prod = this.prod + timeScale
      }
      if ((this.phi + l * Math.PI / 180) * 180 / Math.PI < 0.1) {
        this.phi = 0.1 * Math.PI / 180
      } else {
        this.phi = this.phi + l * Math.PI / 180
      }
      // this.omega = this.prod * Math.sqrt(this.elasticK / this.mass)
      // this.period = 1 * 2 * Math.PI / this.omega
    },
    increaseA (l, timeScale) {
      if (this.prod + timeScale < 0.1) {
        this.prod = 0.1
      } else {
        this.prod = this.prod + timeScale
      }
      if (this.amplitude + l < 1) {
        this.amplitude = 1
      } else {
        this.amplitude = this.amplitude + l
      }
      // this.omega = this.prod * Math.sqrt(this.elasticK / this.mass)
      // this.period = 1 * 2 * Math.PI / this.omega
    }
