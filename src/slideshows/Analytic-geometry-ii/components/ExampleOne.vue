  ...
  .eg-slide-content
    ...
    .center
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 300" height="300px" width="600px" >
        ...
        <!-- plots -->
        <!-- directriz -->
        <line v-if="enterPotencia == 2 & showD" x1="0" :y1="150 + deltaY*enterP" x2="300" :y2="150 + deltaY*enterP" fill="none" stroke="#0AF" stroke-width="3" stroke-linecap="round"/>
        <line v-if="enterPotencia == 1 & showD" :x1="150 - deltaX*enterP" y1="0" :x2="150 - deltaX*enterP" y2="300" fill="none" stroke="#0AF" stroke-width="3" stroke-linecap="round"/>
        
        <!-- Parabola -->
        <path v-if="enterPotencia == 2 || enterPotencia == 1" :d="parabola" stroke="#000" fill="none" stroke-opacity="1" stroke-width="3" ></path>

         <!-- Lado recto -->
        <line v-if="showLR & enterPotencia == 2" :x1="150 - 2*deltaX*enterP" :y1="150 - deltaY*enterP" :x2="150 + 2*deltaX*enterP" :y2="150 - deltaY*enterP" fill="none" stroke="#0AF" stroke-width="3" stroke-linecap="round"/>
        ...
        <circle v-if="showLR & enterPotencia == 2" r="2" :cx="150 + 2*deltaX*enterP" :cy="150 - deltaY*enterP"  fill="#0af" stroke="#0aF" stroke-width="2"/>
        <circle v-if="showLR & enterPotencia == 2" r="2" :cx="150 - 2*deltaX*enterP" :cy="150 - deltaY*enterP"  fill="#0af" stroke="#0aF" stroke-width="2"/>
        <circle v-if="showLR & enterPotencia == 1" r="4" :cx="150 - deltaX*enterP" :cy="150 - 2*deltaY*enterP"  fill="#000" stroke="#F00" stroke-width="2"/>
        <circle v-if="showLR & enterPotencia == 1" r="4" :cx="150 - deltaX*enterP" :cy="150 - 2*deltaY*enterP"  fill="#000" stroke="#F00" stroke-width="2"/>
        ...
        <!-- texto L(-p,y) -->
        <text v-if="enterPotencia == 2 & showD" x="170-deltaX*enterP" :y="170 + deltaY*enterP" font-size="20" fill="#00F" font-weight="bold" font-family="Times" style="font-style: italic;">L(x, <tspan style="font-style: italic;">-p</tspan>)
        </text>
        <text v-if="enterPotencia == 1 & showD" :x="160-deltaX*enterP" y="50 " font-size="20" fill="#00F" font-weight="bold" font-family="Times" style="font-style: italic;">L(<tspan style="font-style: italic;">-p</tspan>, x)
          //- <animate attributeName="x" :values="parabolaX" dur="5" repeatCount="indefinite"/>
        </text>

      </svg>
    //- p.center {{ showLR }}, {{ enterP }}, R={{ 150 - 2*deltaX*enterP }}<br>X1={{ 150 + 2*deltaX*enterP}},Y1={{ Y1 }} 
    .center
...
export default {
  ...
  computed: {
    ...
    A: function () {
      return this.sgA * this.a
    },
    B: function () {
      return this.sgB * this.b
    },
    LR: function () {
